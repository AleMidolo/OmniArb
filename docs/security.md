...
| Replayed/duplicate Stripe events | Signature timestamp tolerance; unique `(provider, eventId)`; operation-level deduplication by event type/object/transition | Return `2xx` once an event is durably accepted, including duplicates; outbox retries and reconciliation repair unfinished state |
...
| Setup-session/customer confusion | Server-selected customer and fixed return URLs; durable setup operation; reconcile setup mode, expected customer, succeeded SetupIntent and attached reusable PaymentMethod; reject browser provider IDs | Cross-flow/cross-customer contract tests; alert correlation conflicts |
...
| Webhook endpoint abuse/DoS | Request size/time limits; signature check before domain work; narrow event allowlist; short durable-accept transaction; provider retrieval and domain work in asynchronous workers | Rate/anomaly monitoring; provider delivery dashboard and recovery runbook |
